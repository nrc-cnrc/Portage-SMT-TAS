...
Here, we show how you can configure a simple crontab job to delete, at the first hour of every day, all workspaces that have not been used in `keep_time` days and that have not recieved new sentence pairs in `keep_time` days.
...
keep_time=7  # as in n*24 hours ago.
context=/var/www/html/plive

# This computes the intersection of directory names of unused workspaces with directory names of workspaces that did not recieved any new incremental sentence pairs.
0 1 * * * find $contexts \( -name canoe.ini.cow -atime +$keep_time -or -name corpora -atime +$keep_time \) -exec dirname {} \; | sort | uniq --repeated | xargs rm -fr
------------------------

*NOTE* the crontab entry __must__ be on a single line.
