...
* Alternative to preprocessing: link:TMXProcessing.html[Extracting text from a TMX]

Preeprocessing must be performed first, but the order of
later steps can vary depending on the application. The
link:PortageIIFileFormats.html[File Formats] section
...
link:PortageIIFileFormats.html#trueplain-text[Plain Text]
...
`preprocess_plugin` used by PortageLive.
...
better processing by the rest of the PortageII suite.
...
link:PortageIIFileFormats.html#trueplain-text[Plain Text] from
your TMX.
...
easiest way to build your training corpus for PortageII.

Tokenizing
^^^^^^^^^^

Tokenizing converts
link:PortageIIFileFormats.html#trueplain-text[Plain Text] into
link:PortageIIFileFormats.html#truetokenized-text[Tokenized Text]
by splitting running text into whitespace-separated tokens, and
...
link:PortageIIFileFormats.html#truetokenized-text[Tokenized
...
Example using Moore's aligner (not supplied with PortageII):
...
link:PortageIIFileFormats.html#truemarked-up-text[Marked Up
...
PortageII includes software to use such markup to reliably
...
`predecode_plugin` for use with PortageLive uses marked-up text to
...
used. (See link:PortageLiveCustomizationPlugins.html[PortageLive Customization
Plugins] for details.)
...
PortageII now assume it, including `translate.pl` and the
PortageLive API.
...
programs in PortageII treat the data as a byte stream, and are
