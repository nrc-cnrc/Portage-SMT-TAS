...
phrase_oracle [-Hvx][-a 'meth args'][-c cf][-ibm n][-m maxlen][-min minlen][-d ldiff][-p pt]\n\
...
-x   Write tgt phrases first in output table. Use this if tgt language code\n\
     (eg 'en', 'fr', etc) comes before the src language code in lexicographic order,\n\
     to maintain the standard convention for joint phrasetables. This also causes -p\n\
     to expect tgt phrases first. [write src first]\n\
-a   Main word-alignment method and optional args. Use -H for help. [IBMOchAligner 2]\n\
-c   Interpolate primary phrases with Cartesian product over phrase pairs, using\n\
     a coefficient of cf on the cartesian pairs (and 1-cf on primary pairs) [0]\n\
...
-d   Max permissible difference in # of words between source and target phrases. [4]\n\
-p   Restrict output phrases to ones that occur in <pt>, which may be either joint\n\
     or conditional, but must have src phrases in left column (unless -x given).\n\
\n\
";
...
int main(int argc, char* argv[])
{
   ...
   if (ibm_src_given_tgt->trainedWithNulls())
      ibm_src_given_tgt->useImplicitNulls = true;
   if (ibm_tgt_given_src->trainedWithNulls())
      ibm_tgt_given_src->useImplicitNulls = true;
