 ...
 *
 *
 * COMMENTS: determines Levenshtein alignment for two sequences of something
 * (string, chars,...) sequences are represented through vectors
 * either distance or a matrix with the alignment is returned
 *
 * MORE COMMENTS: this file contains the definitions of the functions as well
 * because the compiler will output funny linker errors if a separate .cc or
 * .cpp file is created
 *
...
/// represent correct words, substitution, insertion and deletion through
/// 'char'.
const char corr=0, sub=1, del=2, ins=3;


/**
 * Class for backpointers storing the best predecessor in the Levenshtein
 * alignment.
 * Elements: indices of best predecessor in the Levenshtein alignment matrix:
 *           j=row, i=column
 *           char indicating correct (0), substitution (1), deletion (2),
 *           insertion (3), dummy for initialization (4)
 */
class bkp {
 public:
  int  j,i;
...
public:
  ...
  void        dumpAlign(const Tv &hyp, const Tv &ref,
                        const vector<int>& align, ostream& os);
...
/**
 * Determine Levenshtein alignment between two sequences and store information
 * in 'Book' Note: Book[j] is the j-th column.
 *
                                ...
                                const vector<int>* del_costs)
{
  ...
  /**
   * store current column of the Levenshtein matrix in 'Q', and previous column
   * in 'Q_old' the other information is not needed in Dynamic Programming :-)
...
/**
 * Determine Levenshtein alignment between two sequences and return a vector
 * indicating the positions which the elements of 'hyp' are aligned to in 'ref'.
 * Note: positions start at 0
 *   -1 indicates deletion of the element
 *   inserted elements (in 'ref') are stored in the vector entries >=
 *   hyp.size() [is that a good idea? GF: probably not; it's just cost me a
 *   core dump, because I forgot about this trick, and assumed that the
 *   returned alignment would be the same length as hyp - so maybe change it
 *   some day :-)]
...
/**
 * Determine Levenshtein alignment between two sequences and return the
 ...
 * Return the minimal Levenshtein distance which is obtained by covering only
 ...
 */
template<class T>
 void Levenshtein<T>::dumpAlign(const Tv &hyp, const Tv &ref,
                                const vector<int>& align, ostream& os)
{
   Uint next = 0;
