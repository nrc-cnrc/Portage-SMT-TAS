...
one-segment-per-line format (but see options C<-xml> and C<-ttx>).
...
=item -nl=s           mark the end of a sentence [default if -xml or -notok];
...
=item -xml            Input/output text in XML format (not compatible with C<-train>)

=item -ttx            Input text in TTX format, output in plain text (not compatible with C<-train>) See caveats in C<ce_ttx2ospl.pl -help>.

=item -xsrc=S         TTX/XML source language name [EN-CA]

=item -xtgt=T         TTX/XML target language name [FR-CA]

=item -out=F          Output CE values to file F [stdout]

=item -filter=T       Filter out translations below confidence threshold T (C<-xml> mode only)
...
our($desc, $tmem, $train, $plugin,
    $test, $src, $tgt, $xml, $ttx, $xsrc, $xtgt, $k, $norm, $dir, $path,
