...
class Translation;   // Sentence + PhraseAlignment
...
class PhraseAlignment;     // vector<AlignedPair>
...
public:
   ...
   static bool sourceLessThan(const AlignedPair &p1, const AlignedPair &p2) {
      if (p1.source.first == p2.source.first)
         return (p1.target.first < p2.target.first);
      else
   ...
   static bool targetLessThan(const AlignedPair &p1, const AlignedPair &p2) {
      if (p1.target.first == p2.target.first)
         return (p1.source.first < p2.source.first);
      else
...
class PhraseAlignment : public vector<AlignedPair> {
public:
   /// Constructor.
   PhraseAlignment() {}
   /// Destructor.
   ...
   Uint sourceLength() const;
   Uint targetLength() const;
...
public:
   ...
   Translation(const string& t = "") : Sentence(t) {}
   virtual ~Translation();
   // phraseAlignment is mutable in order to sort on target/source.
   mutable PhraseAlignment phraseAlignment;  ///< associated alignment for this translation

   void write(ostream& out) const;
};


/// A reference is a translation which is a sentence.
typedef Translation Reference;
