...
int MAIN(argc, argv)
{
   ...
   for (; pfr->pollable(); ++s)
   {
      ...
      if (!(arg.bBestSentence || arg.bWorseSentence || arg.bDistribution || arg.bVerbose || arg.outopt != "std"))
         nbests[s].clear();
   }
   ...
   for (vUcit it(arg.numBest.begin()); it!=arg.numBest.end(); ++it) {
      ...
      if (arg.outopt == "std") {
         ...
         if (arg.bCi) {
            const double score = exp(best.score());
            const double gamma = bootstrapConfInterval(bleu.begin(), bleu.end(),
                  BLEUcomputer(*it), 0.9, 100);
         ...
         for (Uint s = 0; s < arg.S; ++s) {
            best -= bleu[s][bestIndex[s]];
            for (Uint k = 0; k < nbests[s].size(); ++k) {
               const BLEUstats& t = bleu[s][k];
               best += t;
               cout << s << ' ' 
                  << exp(t.score()) << ' '
                  << exp(best.score()) << endl;
