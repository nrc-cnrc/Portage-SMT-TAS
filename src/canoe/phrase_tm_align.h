...
namespace Portage
{
	...
	public:
	     ...
	     * c.distLimit   The maximum distortion distance allowed between two words,
	     *               or NO_MAX_DISTORTION for no limit.
             * c.verbisity   The verbosity level (1 to 4) [1 -- quiet]
	     */
	    PhraseTMAligner(const CanoeConfig& c,
                            const string &phraseTableFile); 
	    
	    /**
	     * Creates a new PhraseTMAligner, optionally limiting the phrases loaded to
	     * ones in a given source sentence.
             * @param c  canoe config
	     * @param phraseTableFile	The file containing the phrase table.
	     * c.distWeight     The distortion weight, relative to a weight of 1.0
	     *                  for the translation model.
	     * c.distLimit	The maximum distortion distance allowed between two words,
	     * 			or NO_MAX_DISTORTION for no limit.
	     * @param src_sents	If limitPhrases is true, the phrases are limited to those
	    ...
	    /**
	     * Maximizes the weighted distortion + translation model score over all
	     * alignments from src_sent to tgt_sent, constrained by distLimit.
