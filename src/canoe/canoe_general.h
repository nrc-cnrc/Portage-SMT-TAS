 ...
 * @file canoe_general.h  This file contains commonly used structures and simple functions.
 * 
 * $Id$
 * 
 * Canoe Decoder
...
namespace Portage
{
    /// What Portage think is the value of log of almost 0
    extern const double LOG_ALMOST_0;
    /// What Portage think is the value of almost infinity
     ...
     * end = first word NOT in the range, which must be <= the number of words in the
     *          sentence
     * Hence, the length of the range is simply (end - start). If the empty range is
     * not being allowed (which is generally the case), then end > start.  The empty
     * range is given when end == start.  end < start is always invalid.
     */
    struct Range
    {
	...
	bool operator<(const Range &other) const
	{
         ...
         * Equality operator.
         * @param other right-hand side operand.
         * @return Returns true if both Ranges starts and ends at the same indexes.
         */
	bool operator==(const Range &other) const
	{
    ...
    /**
     * A set of Uint's is represented uniquely by a minimal set of Range's which form a
     * partition of the set; this partition set in turn is uniquely represented as a
     * sorted vector of the Range's.  All functions which use UintSet have these
     * restrictions (minimal partition size and orderedness) as pre- and post-conditions.
     */
    typedef vector<Range> UintSet;
    
    /**
     * Creates the union of the set s of Uint's with all the Uint's in the range r.  The
    ...
    /**
     * Creates the set subtraction, the set s of Uint's minus all the Uint's in the range
     * r.  The result is stored in result.
     * @param result	The result of the operation is stored here.
     * @param s	A set of Uint's.
     * @param r	A contiguous set of Uint's
     */
    ...
    /**
     * Creates the set intersection, the set s of Uint's intersect all the Uint's in the
    ...
    /**
     * Given an item associated with each subrange of some range [0, R), and given a
     * UintSet, this picks out all items whose associated range is a subset of the Uint
     * set.
     ...
     */
    template <class T>
    inline void pickItemsByRange(vector<T> &result, T **triangArray, const UintSet &set)
    {
   ...
   /**
   * Callable entitiy that creates a triangular array, a, of objects of type T.
   * The array has length size, and the i-th entry has (size - i) elements.  This
   * is precisely the dimension needed for the return value of
   * PhraseDecoderModel::getPhraseInfo() when size = source sentence length, for
   * example.
   ...
   */
   ...
   class CreateTriangularArray
   {
      public:
         /**
            * Creates a triangular array that must be deleted by its user.
            * @param size  size of the triangular array.
            * @return Returns the triangular array.
            */
 ...
 * @param size  maximum number of elements to use for the dot product 
 */
template<class T, class U>
T dotProduct(vector<T> a, vector<U> b, Uint size)
{
    assert(size <= a.size());
    assert(size <= b.size());
    T result = (T)0;
