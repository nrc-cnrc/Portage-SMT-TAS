...
/**
 * A toy PhraseDecoderModel for the test.  The score (and future score) assigned to a
 * translation is precisely the first word in its last phrase (words is represented by
 * unsigned ints).  All other functions of the PhraseDecoderModel are not used by
 * runDecoder().
 */
...
/**
 * A toy HypothesisStack used for the test.  Hypotheses are never recombined and never
 * pruned.
 */
class TestHypStack: public HypothesisStack
{
	...
	{
	    for (vector<DecoderState *>::const_iterator it = v.begin(); it !=
		    v.end(); it++)
	    {
		assert(*it != NULL);
...
/**
 * A toy PhraseFinder used for the test.  The phrase options returned by findPhrases is
 * determined by the first range in the sourceWordsNotCovered, and the mapping is
 * prespecified using a hash_map.
 */
class TestFinder: public PhraseFinder
{
    private:
        /// Definition that associates Range with a PhraseInfo
	hash_map<Range, vector<PhraseInfo *>, RangeHash> &map;
	
    public:
        /// Constructor.
        /// @param map
	TestFinder(hash_map<Range, vector<PhraseInfo *>, RangeHash> &map): map(map) {}
...
*/
    ...
    RangeHash rangeHash;
    hash_map<Range, vector<PhraseInfo *>, RangeHash> finderMap(10, rangeHash);
