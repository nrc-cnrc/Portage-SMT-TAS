...
/**
 * A toy PhraseDecoderModel for the test.  Phrases are considered recombinable if they
 * cover the same number of source words.  Functions other than computeRecombHash and
 * isRecombinable should not be used by HypothesisStack objects.
 */
...
void print(HypothesisStack *stack)
{
	...
	cout << "Recombs: " << flush;
	for (vector<DecoderState *>::const_iterator it = cur->recomb.begin(); it
		!= cur->recomb.end(); it++)
	{
	    cout << (*it)->score << flush;
	} // for
...
/*
Eqivalence classes (using the "is recombinable" equivalence relation).  Since two
states are considered recombinable iff they have the same number of source words covered,
each equivalence class is specified by the number of source words covered.  In brackets is
the score given to the state.
...
*/
    ...
    // In order to produce output after every state added to the stack, we need to create
    // the stack from scratch after each output.
