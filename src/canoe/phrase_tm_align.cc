...
Each source sentence may have a fixed number of contiguous translations in the\n\
...
Options:\n\
 ...
 -segmentation-args  args\n\
                       The segmentation model arguments (see canoe for choices and default).]\n\
...
int main(Uint argc, const char * const * argv)
{
    ...
    argReader.testAndSet("distortion-model", distModel);
    argReader.testAndSet("weight-d", c.distWeight[0]);
    argReader.testAndSet("segmentation-model", segModel);
    argReader.testAndSet("segmentation-args", segArgs);
    ...
    if (distModel!="") {
      vector<string> toks;
      split(distModel, toks, ":", 2);
      c.distModelName = toks[0];
      if (toks.size() == 2) c.distModelArg = conv<Uint>(toks[1]);
    }
    cerr << "Distortion model: " << c.distModelName << " with weight " << c.distWeight[0] << endl;
    if (segModel!="") {
      c.segModelName = segModel;
      c.segModelArgs = segArgs;
    }
    cerr << "Segmentation model: " << c.segModelName << " with weight " << c.segWeight[0] << endl;
    ...
    if (!c.loadFirst)
    {
        ...
        if (argReader.getSwitch("K"))
        {
            if (tgt_sents.size() / src_sents.size() != K)
            {
                ...
                error(ETWarn, "K specified on command-line does not match computed K.  Using %d.",
                        K);
            } // if
        } else
        {
    ...
    while (true)
    {
        ...
        for (Uint k = 0; k < K; k++)
        {
            ...
            string s = ss.str();
            //s contains the alignments

            //print out original sentence
            if (!onlyscore) {
