...
ForcedTargetPhraseFinder::ForcedTargetPhraseFinder(PhraseDecoderModel &model, const
        vector<string> &tgt_sent, int distLimit): srcLength(model.getSourceLength())
{
        ...
        for (Uint j = 0; j < srcLength; j++)
        {
            for (Uint k = 0; k < srcLength - j; k++)
            {
                        ...
                        phrases[j][k].begin(); it < phrases[j][k].end(); it++)
                {
                    ...
                    for (Uint l = 0; match && l < (*it)->phrase.size(); l++)
                    {
                        match = ( (*it)->phrase[l] == uint_tgt_sent[i + l] );
                    } // for
                    if (match)
                    {
// 		       string src, tgt;
