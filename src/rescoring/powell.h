   ...
   class Powell {
      ...
      public:
         ...
         /**
            ...
            - For each s = 0, .. , S-1, we have candidate translations
              e_{s,0}, .. , e_{s,K-1} (eg. K-best)
            - For each candidate translation, statistics for the score.  These
              statistics should be additive - the total score for a set of
              translations is determined by adding up the statistics then
              computing the score from the sum.  It is assumed that greater
              allScoreStats are better; if the reverse is true, just use the
              negative score instead.
            - Feature functions h_m(e,f) for m = 0, .. , M-1

            Parameters to this function is as follows:
            p - A vector containing the starting point in the parameter space.
                The "optimal" parameters found will be returned via p.
            U - The directions used in Powell's algorithm.  Generally, the M x
                M identity is a good starting value for this matrix.  The
                contents of the matrix will be modified by the algorithm.
            tolerance - The fractional tolerance in the function value, such
                        that when one iteration does not increase the score by
                        at least this much, then the algorithm terminates.
            TODO: consider whether fractional tolerance is appropriate, or if
                  some other tolerance should be used.
            iter - Will contain the number of iterations done in the algorithm.
            score - Will contain the maximum score achieved.
            H - An array of length S, of K x M matrices containing the
                evaluation of the feature functions at the candidate
                translations.  Specifically, the (k, m)-th entry of H[s] should
                contain the value h_m(e_{s,k}, f_s).
            bleu - An S x K array of score statistics for the candidate
                   sentences.  Suggestively called "bleu", though it is easy to
                   write a class for other types of allScoreStats, so long as
                   the score can be broken into linear components.  The
                   following must be defined for the type ScoreStats:
            ...
            By default, a new object of type ScoreStats must have "zero"
            statistics - that is, it should represent the score given to an
            empty set of translations.
            See bleu.h, bleu.cc (in eval) for an example of the implementation
            of the BLEU statistics.  bleu[s][k] should contain the score
            statistics assigned to the sentence e_{s,k}.  Of course, the score
            statistics is not a function only of e_{s,k} but (presumably) of
            the correct translation as well.
